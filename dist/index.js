...
var vueheap = (function() {
  'use strict';

  var HEAP_INSTANCE_METHODS = [
    'addEventProperties',
    'addUserProperties',
  ...
  var VueHeap = {
    createHeap: function createHeap(key, config) {
      // initialize heap if necessary
      window.heap = window.heap || []; // set configuration properties

      window.heap.appid = key;
      window.heap.config = config || {}; // while we are waiting for the heap script to load we still need to be
      ...
      for (var i = 0; i < HEAP_INSTANCE_METHODS.length; i++) {
        _loop(i);
      }
    },
    addHeapScript: function addHeapScript(key, config) {
      var useHTTPS = config.forceSSL || 'https:' === document.location.protocol;
